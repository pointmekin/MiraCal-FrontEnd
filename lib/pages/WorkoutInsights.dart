...
import 'package:flutter_app/components/workoutCalCardNoTouch.dart';
...
class WorkoutInsights extends StatefulWidget {
  @override
  _WorkoutInsightsState createState() => _WorkoutInsightsState();
}
...
class _WorkoutInsightsState extends State<WorkoutInsights> with SingleTickerProviderStateMixin{
  ////////////////////////////
...
*/
  ...
  void initState() {
    ...
    getApplicationDocumentsDirectory().then((Directory directory) async {
      ...
      setState(() {
        tempWorkoutListDateKey = result2;
        calEatenToday = fileContent[calEatenDateKey];
        calBurnedToday = fileContent[calWorkoutDateKey];
      ...
      body: fileContent == null ? Container() : SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                children: [
                  ...
                  fileContent[calWorkoutDateKey] == null || fileContent[workoutListDateKey] == null
                      ? Column(
                    children: [
                      Text("You have not done any workout today."),
                      SizedBox(height: 10),
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Text('Net Calories burned today'),
                        ],
                      ),

                      Row(
                        children: [
                          Text(calBurnedToday.toString(), style: TextStyle(fontSize: 40, fontWeight: FontWeight.bold, color: Palette.miraCalPink),),
                          Text(" Cal"),
                          /*
                      ...
                      Row(
                        children: [
                          Text("Workouts Done Today", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),),
                        ],
                      ),
                      SizedBox(height: 10,),
                      tempWorkoutListDateKey == null ? Container() :  ListView.builder(
                        physics: NeverScrollableScrollPhysics(),
                        //controller: _controller,
                        scrollDirection: Axis.vertical,
                        shrinkWrap: true,
                        itemCount: tempWorkoutListDateKey.length,
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Container(
                                          ...
                                          child: tempWorkoutListDateKey == null ? Container() : Text(

                                            tempWorkoutListDateKey[index][0].substring(0,getCuttingIndex(tempWorkoutListDateKey[index][0])), style: TextStyle(fontSize: 18),)
                                      ),
                                      Text(tempWorkoutListDateKey[index][0].substring(getCuttingIndex(tempWorkoutListDateKey[index][0])) + " Cal", style: TextStyle(fontSize: 18),),
                                      IconButton(
                                        ...
                                        onPressed: (){
                                          setState(() {
                                            ...
                                            jsonFileContent[calWorkoutDateKey] = (double.parse(fileContent[calWorkoutDateKey]) - double.parse(tempWorkoutListDateKey[index][0].substring(getCuttingIndex(tempWorkoutListDateKey[index][0])))).toString();
                                            calBurnedToday = (double.parse(calBurnedToday) - double.parse(tempWorkoutListDateKey[index][0].substring(getCuttingIndex(tempWorkoutListDateKey[index][0])))).toString();

                                            tempWorkoutListDateKey.remove(tempWorkoutListDateKey[index]);

                                            if (tempWorkoutListDateKey.length == 0) {
                                              setState(() {
                                                //print(tempWorkoutListDateKey);
                                                dailyInfo.calBurnedToday = 0;
                                                jsonFileContent.remove(calWorkoutDateKey);
                                                jsonFileContent.remove(workoutListDateKey);
                                              });
                                            } else {
                                              jsonFileContent[workoutListDateKey] = tempWorkoutListDateKey.toString();
                                              //print(tempWorkoutListDateKey);
                                              //jsonFileContent[foodEatenDateKey] = tempWorkoutListDateKey.toString();
                                            }
                                            ...
                                            dailyInfo.calBurnedToday = int.parse(double.parse(calBurnedToday).round().toString());
            ...
            // this button redirects to the workout search page
            /*
            Container(
              child: Column(
                children: [
                  ...
                  SizedBox(height: 30,),
// This is for testing, displaying all content in the jsonfile
                /*
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal:30),
